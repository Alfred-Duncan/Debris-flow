...
def main(args):
    ...
    effect,ready=classify(methods['FrozenGlobal'],methods,preservation)
    out=ROOT/'results/local_corrector_support_guard';out.mkdir(parents=True,exist_ok=True)
    ...
    report={'baseline_sha':'bdcbb22ace3daa3655ef0e502910b3a325828f76','local_checkpoint':'best.pt','local_checkpoint_update':checkpoint['update'],'scope':'VAL_ONLY','support_rule':'active AND (current_h >= 0.05 OR global_provisional_h >= 0.05)','methods':methods,'runtime':timings,'SUPPORT_GUARD_EFFECT':effect,'LOCAL_CORRECTOR_READY_TO_FREEZE':ready}
    (out/'support_guard_report.json').write_text(json.dumps(report,indent=2,allow_nan=True));(ROOT/'reports/LOCAL_CORRECTOR_SUPPORT_GUARD.json').write_text(json.dumps(report,indent=2,allow_nan=True))
    print(json.dumps({'status':'PASS','SUPPORT_GUARD_EFFECT':effect,'LOCAL_CORRECTOR_READY_TO_FREEZE':ready,'output':str(out)},indent=2))

if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--limit',type=int);main(parser.parse_args())
