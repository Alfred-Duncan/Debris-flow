...
from scripts.generate_formal_scenarios import case_config,solver_cmd,retain_ten_second_frames,qa
...
def generate():
 ...
 for _,row in d.iterrows():
  ...
  case.mkdir(parents=True,exist_ok=True);(case/'config.json').write_text(json.dumps(case_config(row),indent=2));cp=subprocess.run(solver_cmd(row,case),cwd=ROOT,check=True);frames=retain_ten_second_frames(case);quality=qa(case,case_config(row),float('nan'));quality['frames_saved']=frames;(case/'quality.json').write_text(json.dumps(quality,indent=2));
  if quality['status']!='PASS':raise RuntimeError(f'holdout QA failed: {row.scenario_id}')
  (case/'DONE.json').write_text(json.dumps({'status':'PASS','scenario_id':row.scenario_id,'sealed':True}))
