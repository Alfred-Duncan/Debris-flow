...
DRY=.03
def project_physical(x,dry:float=DRY):
    y=x.clone();y[:,0].clamp_(min=0);y[:,3:5].clamp_(0,1);tot=y[:,3]+y[:,4];over=tot>1;y[:,3][over]/=tot[over];y[:,4][over]/=tot[over]
    drymask=y[:,0:1]<dry;y[:,1:3]*=(~drymask).to(y.dtype);y[:,3:5]*=(~drymask).to(y.dtype);return y
def teacher_weight(current,target,active,dry:float=DRY):
...
def wet_loss(pred_p,target_p,active,tau=.02):
    p=torch.sigmoid((pred_p[:,0]-DRY)/tau)*active;q=torch.sigmoid((target_p[:,0]-DRY)/tau)*active
