"""Isolated Park-v2 documented reimplementation.

This copy preserves v1 numerics except the v2 Eq. 2--3 density and phase-change
updates documented in the EarthArXiv Version 2 manuscript. The author v2 solver
source itself is not publicly released.
"""
...
class SparseSWE:
    ...
    def step(self, dt: float) -> None:
        ...
        z = self.z
        if self.melt_energy:
            # Park v2 Eq. 2: rho_m=1000+1650*c-1750*i (kg m^-3).
            rho0 = 1000.0 + 1650.0 * c - 1750.0 * ice
        ...
        if self.melt_energy:  # energy leaving through the domain edge is not dissipation
            ...
            ie = (hi_new[self.edge] / he).clamp(0.0, 1.0)
            rhoe = 1000.0 + 1650.0 * ce - 1750.0 * ie
            e_edge = (rhoe * (0.5 * (hu_new[self.edge] ** 2 + hv_new[self.edge] ** 2) / he + G * h_new[self.edge] * (z[self.edge] + 0.5 * h_new[self.edge]))).sum()
        ...
        if self.eta0 > 0:  # viscous (laminar) resistance of the mixture, K eta u / (8 rho_m h^2), Julien & Lan (1991)
            ...
            inew = (hi_new / hs).clamp(0.0, 1.0)
            rhon = 1000.0 + 1650.0 * cn - 1750.0 * inew
            turb = turb + dt * 3.0 * eta / (rhon * hs * hs)
        ...
        if self.melt_eff > 0 or self.melt_tau > 0:
            ...
            if self.melt_energy:
                rho2 = 1000.0 + 1650.0 * cn - 1750.0 * (hi_new / hs).clamp(0.0, 1.0)
            ...
            # Park v2 Eq. 3: 1 m3 ice becomes 0.9 m3 water; retain velocity.
            h_before_melt = h_new
            h_new = (h_new - 0.1 * dm).clamp_min(0.0)
            velocity_scale = h_new / h_before_melt.clamp_min(self.h_dry)
            hu_new = hu_new * velocity_scale; hv_new = hv_new * velocity_scale
            hq_new = (hq_new - dm * LAT).clamp_min(0.0); hi_new = hi_new - dm; hc_new = (hc_new - dm).clamp_min(0.0)
            wet = h_new > self.h_dry; hs = h_new.clamp_min(self.h_dry)
            cn = (hc_new / hs).clamp(0.0, 1.0)
