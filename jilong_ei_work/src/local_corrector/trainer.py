...
def apply_learned_correction(model, features, encoded_provisional, encoded_current, patches, layout,
                             scales, bounds, active, global_model=None):
    """Infer fixed-shape patches, then make a core-only transformed update."""
    inputs = local_features(features, encoded_provisional, encoded_current, scales, patches, layout)
    raw = model(inputs)
    correction, normalized = bounded_correction(core(raw, layout), scales, bounds)
    ...
    nb = torch.as_tensor(bounds, device=raw.device, dtype=raw.dtype)[None, :, None, None] / torch.as_tensor(scales, device=raw.device, dtype=raw.dtype)[None, :, None, None]
...
def correction_loss(model, features, provisional_t, truth_t, current_t, provisional_p, truth_p,
                    patches, layout, scales, bounds, active, change_threshold, global_model=None):
    ...
    corrected, prediction, saturation = apply_learned_correction(
        model, features, provisional_t, current_t, patches, layout, scales, bounds, active, global_model)
    targets = torch.cat([core(extract(truth_t - provisional_t, patch, layout), layout) for patch in patches], dim=0)
    target = targets / torch.as_tensor(scales, device=targets.device, dtype=targets.dtype)[None, :, None, None]
    ...
    current = torch.cat([core(extract(current_t, patch, layout), layout) for patch in patches], dim=0)
    wet = (truth[:, :1] >= .05) | (prov[:, :1] >= .05)
    fp = (prov[:, :1] >= .05) & (truth[:, :1] < .05)
    changed = (truth - current).abs().mean(1, keepdim=True) >= float(change_threshold)
    ...
    return huber + .01 * magnitude, {"huber": float(huber.detach()), "mag": float(magnitude.detach()),
