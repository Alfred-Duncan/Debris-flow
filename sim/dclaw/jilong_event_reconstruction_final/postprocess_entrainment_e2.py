...
OUT=CASE/"runs/C3_entrainment_E2/_output"
...
def main():
    ...
    sane=finite and maxh<100. and maxv<40.;s1=arrivals["S1"]["0.001"];gate="PASS" if sane and s1 is not None and s1<=900. else "FAIL";classification="E2_MEANINGFUL_DOWNSTREAM_MOBILITY" if gate=="PASS" else ("E2_ENTRAINMENT_INSUFFICIENT" if sane else "E2_NUMERICAL_PATHOLOGY")
    result={"scenario":"C3_entrainment_E2","completed_to_s":frames[-1]["time_s"],"implementation_repair_reruns_used":0,"source_support_cells":len(source["cells"]),"terrain_modified":False,"source_modified":False,"material_baseline_modified":False,"entrainment_rate":.20,"available_layer_thickness_m":2.,"h_e_aux_index_fortran":7,"numerical_gate":{"finite":finite,"max_depth_lt_100":maxh<100.,"max_speed_lt_40":maxv<40.},"max_depth_m":maxh,"max_speed_ms":maxv,"p99_wet_speed_ms":p99,"maximum_bdif_m":maxbd,"E2_GATE":gate,"classification":classification,"arrivals_s":arrivals,"checkpoint_metrics":{str(k):grid[k] for k in sorted(CHECK)},"frames":frames}
    ...
    with (res/"E2_mass_ledger.csv").open("w",newline="") as f:w=csv.DictWriter(f,fieldnames=cols);w.writeheader();w.writerows(frames)
    with (res/"E2_front_trajectory.csv").open("w",newline="") as f:w=csv.DictWriter(f,fieldnames=["time_s","unrestricted_max_projected_chainage_m","max_depth_m","max_speed_ms","p99_wet_speed_ms","entrained_volume_m3","total_wet_volume_m3"]);w.writeheader();w.writerows([{k:r[k] for k in w.fieldnames} for r in frames])
    with (res/"E2_section_arrivals.csv").open("w",newline="") as f:
    ...
    plt.figure(figsize=(7,4));plt.plot(times,front/1000,"o-");plt.xlabel("Time (s)");plt.ylabel("Source-connected front chainage (km)");plt.grid();plt.tight_layout();plt.savefig(fig/"E2_front_trajectory.png",dpi=180);plt.close()
    plt.figure(figsize=(7,4));plt.plot(times,ev/1e6,"o-");plt.xlabel("Time (s)");plt.ylabel("Entrained volume from bdif (10^6 m3)");plt.grid();plt.tight_layout();plt.savefig(fig/"E2_entrained_volume.png",dpi=180);plt.close()
    (rep/"ENTRAINMENT_E2_RESULT.json").write_text(json.dumps(result,indent=2)+"\n")
    lines=["# E2 entrainment result","","E2 reached 900 s in its first formal attempt. bdif is D-Claw state component 7 (Fortran one-based); entrained volume is sum of non-negative bdif times 64 times 64 m2.","",f"- Numerical gate: finite={finite}, max depth={maxh:.6g} m, max speed={maxv:.6g} m/s, P99 wet speed={p99:.6g} m/s.",f"- Maximum bdif: {maxbd:.6g} m.",f"- E2 gate: **{gate}**; classification: **{classification}**.","","| t (s) | front chainage (m) | max depth (m) | max speed (m/s) | P99 wet speed (m/s) | entrained volume (m3) | wet volume (m3) |","|---:|---:|---:|---:|---:|---:|---:|"]
    ...
    (rep/"ENTRAINMENT_E2_RESULT.md").write_text("\n".join(lines)+"\n")
    print(json.dumps({k:result[k] for k in ["completed_to_s","max_depth_m","max_speed_ms","p99_wet_speed_ms","maximum_bdif_m","E2_GATE","classification"]},indent=2))
