...
subroutine bc2amr(val,aux,nrow,ncol,meqn,naux,hx,hy,level,time,xlo_patch,xhi_patch,ylo_patch,yhi_patch)
 ...
 real(kind=8)::hxmarg,hymarg,qp,td,tx,ty,xmin,xmax,m0,rhof,grav,dryt,q,qarea,h,u,v,x,pb
 logical,save::loaded=.false.
 real(kind=8),save::sqp,std,stx,sty,sxmin,sxmax,sm0,srhof,sgrav,sdryt
 ...
 if (.not.loaded) then
   ...
   read(11,*) sgrav; read(11,*) sdryt; close(11); loaded=.true.
 endif
 ...
 if (yhi_patch.gt.yupper+hymarg) then
   ...
   if (mthbc(4).eq.0 .and. time.gt.0.d0 .and. time.lt.std) then
     ...
     qarea=qp/((sxmax-sxmin+hx)*(-sty))
     if (qarea.gt.dsqrt(sgrav*sdryt**3)) then
       h=(qarea*qarea/sgrav)**(1.d0/3.d0); u=qarea/h; v=u*sty; u=u*stx; pb=srhof*sgrav*h
       do i=1,nrow
